...
async fn main() -> std::io::Result<()> {
    ...
    // TODO: Get store_id and tenant_id from config or environment
    ...
    // Start HTTP server
    HttpServer::new(move || {
        // Configure CORS - allow frontend on port 7945
        let cors = Cors::default()
            .allowed_origin("http://localhost:7945")
            .allowed_origin("http://127.0.0.1:7945")
            .allowed_origin("http://192.168.2.65:7945") // Network IP for development
            .allow_any_method()
            .allow_any_header()
        ...
        App::new()
            ...
            // Vendor Bill Receiving endpoints
            .service(
            ...
            // Vendor operations (SKU matching, OCR, parsing, sync logging)
            .configure(handlers::vendor_operations::configure)
            ...
            // OCR operations (image preprocessing, multi-pass OCR)
            .configure(handlers::ocr_operations::configure)
            ...
            .configure(handlers::review_cases::configure)
            // Re-OCR and mask endpoints
            .configure(handlers::reocr::configure)
            // Export endpoints
