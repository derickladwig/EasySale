...
ALTER TABLE backup_jobs ADD COLUMN tenant_id TEXT NOT NULL DEFAULT 'default-tenant';
CREATE INDEX IF NOT EXISTS idx_backup_jobs_tenant ON backup_jobs(tenant_id);

-- Add tenant_id to backup_settings
ALTER TABLE backup_settings ADD COLUMN tenant_id TEXT NOT NULL DEFAULT 'default-tenant';

-- Add tenant_id to backup_manifests
ALTER TABLE backup_manifests ADD COLUMN tenant_id TEXT NOT NULL DEFAULT 'default-tenant';
CREATE INDEX IF NOT EXISTS idx_backup_manifests_tenant ON backup_manifests(tenant_id);

-- Add tenant_id to backup_destinations
ALTER TABLE backup_destinations ADD COLUMN tenant_id TEXT NOT NULL DEFAULT 'default-tenant';
...
ALTER TABLE backup_dest_objects ADD COLUMN tenant_id TEXT NOT NULL DEFAULT 'default-tenant';
CREATE INDEX IF NOT EXISTS idx_backup_dest_objects_tenant ON backup_dest_objects(tenant_id);

-- Add tenant_id to restore_jobs
ALTER TABLE restore_jobs ADD COLUMN tenant_id TEXT NOT NULL DEFAULT 'default-tenant';
CREATE INDEX IF NOT EXISTS idx_restore_jobs_tenant ON restore_jobs(tenant_id);

-- Add tenant_id to backup_alerts
ALTER TABLE backup_alerts ADD COLUMN tenant_id TEXT NOT NULL DEFAULT 'default-tenant';
