...
### Changed
- **Login Page Header**: Reduced logo size and removed redundant company name text
  - Logo now constrained to `h-10 max-w-[200px]` for cleaner appearance
  - Company name text only shown when no logo image is configured
- **Backend Cleanup**: Removed unused `QueryBuilder` import from layaway handler

### Fixed
- Login page header logo was oversized when using logo images that include text
